...
# Susceptible genotypes
emmeans(lesion.s, ~Treatment|Genotype) %>% data.frame %>%
    mutate(Genotype=factor(Genotype,levels=levels(susceptible$Genotype)),
           upper.CL=asymp.UCL,
           lower.CL=asymp.LCL) %>%
...
ggsave("output/figs/Fig.4.pdf",width=10,height=4)

# Resistant genotypes
emmeans(lesion.r.noInt, ~Treatment|Genotype) %>% data.frame %>%
  mutate(Genotype=factor(Genotype,levels=levels(susceptible$Genotype)),
         upper.CL=asymp.UCL,
         lower.CL=asymp.LCL) %>%
  ggplot(aes(x=Treatment,y=emmean,fill=Treatment)) +
  ...
  scale_alpha(range=c(0.4,1))+
  labs(y="Disease severity (% rust lesion)")+
  scale_y_continuous(limits=c(0,0.1),labels = function(x) x*100)+
  scale_fill_manual(values=pal.treatment)+
  facet_wrap(~Genotype,nrow=1)+
  theme_few()+
  theme(strip.background = element_blank(),
        ...
        axis.title.x = element_blank(),
        legend.text = element_text(face="italic"))
ggsave("output/figs/Fig.S7.pdf",width=10,height=4)
